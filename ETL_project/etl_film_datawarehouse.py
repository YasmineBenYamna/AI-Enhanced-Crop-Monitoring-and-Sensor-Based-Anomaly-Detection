...
def load_dimension_time(df, conn):
    ...
    for _, row in time_data.iterrows():
        try:
            ...
            errors += 1
            if errors <= 3:
                print(f"   ⚠️ Error inserting TimeID {row['RunTimeMinutes']}: {e}")
...
def load_dimension_film(df, conn):
    ...
    for _, row in df.iterrows():
        try:
            ...
            errors += 1
            if errors <= 5:
                print(f"   ⚠️ Error inserting FilmID {row['FilmID']}: {e}")
    
    print(f"   ✅ Inserted {inserted} film records")
    if errors > 0:
        print(f"   ⚠️ {errors} films had errors")
...
def load_dimension_generic(df, conn, dim_table, id_col, name_col, display_name):
    ...
    for _, row in unique_values.iterrows():
        try:
            ...
            errors += 1
            if errors <= 3:
                print(f"   ⚠️ Error inserting {id_col} {row[id_col]}: {e}")
